...
### 2.3 결정 #3: 다중 통신 방식 지원

> **"통신 방식은 아키텍처의 제약이 아닌 클라이언트의 선택이어야 한다."**
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   클라이언트는 자신의 환경과 요구에 맞는 통신 방식을 선택할 수 있다.    │
│                                                                          │
│   ─────────────────────────────────────────────────────────────────      │
│                                                                          │
│   지원해야 할 통신 방식:                                                 │
...
│                                                                          │
│   ┌─────────────────────────────────────────────────────────────────┐   │
│   │  1. 동기 HTTP (Sync Request-Response)                           │   │
│   │     ─────────────────────────────────                           │   │
│   │     • 요청 → 완료까지 대기 → 전체 응답 수신                     │   │
│   │     • 단순 통합, 짧은 작업에 적합                                │   │
│   │     • 장시간 작업 시 타임아웃 위험 (Edge에서 관리)              │   │
│   └─────────────────────────────────────────────────────────────────┘   │
│                                                                          │
│   ┌─────────────────────────────────────────────────────────────────┐   │
│   │  2. 비동기 HTTP (Async with Job ID)                             │   │
│   │     ─────────────────────────────────                           │   │
│   │     • 요청 → 즉시 Job ID 발급 → Polling 또는 Webhook           │   │
│   │     • 장시간 작업에 적합, 커넥션 점유 최소화                    │   │
│   │     • 기본 권장 방식                                            │   │
│   └─────────────────────────────────────────────────────────────────┘   │
│                                                                          │
│   ┌─────────────────────────────────────────────────────────────────┐   │
...
**핵심 원칙:**
- **클라이언트 선택권**: 시스템이 통신 방식을 강제하지 않음
- **동일한 비즈니스 로직**: 어떤 통신 방식을 사용하든 동일한 파이프라인 실행
- **채널 분리**: 통신 채널과 비즈니스 로직의 독립성 유지
...
> **"시스템은 통신 방식을 강제하지 않는다. 클라이언트가 자신의 환경에 맞는 방식을 선택한다."**
...
│   ───────────                                                            │
│                                                                          │
│   1. 동일한 파이프라인은 어떤 통신 방식을 사용하든 동일하게 실행된다    │
│   2. 통신 채널은 비즈니스 로직과 분리되어야 한다                        │
│   3. 새로운 통신 방식 추가 시 파이프라인 코드 변경이 없어야 한다        │
│                                                                          │
│   ─────────────────────────────────────────────────────────────────      │
│                                                                          │
│   [통신 방식별 특성]                                                     │
...
│   방식          장점                      제약                           │
│   ─────         ─────                     ─────                          │
│   동기 HTTP     단순한 통합               장시간 작업 시 타임아웃        │
│   비동기 HTTP   커넥션 점유 최소화        추가 상태 조회 필요            │
│   SSE           브라우저 호환, 자동 재연결 단방향만 가능                 │
│   WebSocket     양방향, 중간 제어 가능    연결 관리 복잡                 │
│                                                                          │
│   ─────────────────────────────────────────────────────────────────      │
│                                                                          │
│   [구현 원칙]                                                            │
│   ───────────                                                            │
│                                                                          │
│   • API Gateway는 모든 통신 방식에 대해 동일한 Workflow를 시작한다      │
│   • 결과 전달 채널만 클라이언트 선택에 따라 달라진다                    │
│   • 구체적인 구현은 ADR-005에서 결정한다                                │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### 7.2 Dual Path 전략 (Temporal 한계 우회)

| 항목 | 내용 |
|------|------|
| **필요성** | Temporal 한계(Payload 2MB, Event History, Latency)를 우회하면서 통신 방식 유연성 확보 |
| **핵심 원칙** | 클라이언트 통신 프로토콜과 내부 데이터 경로는 **독립적인 관심사** |

**아키텍처 원칙:**
> **"Dual Path는 데이터 특성에 따른 경로 분리이다. 클라이언트 통신 방식 선택과는 독립적이다."**

```
┌─────────────────────────────────────────────────────────────────────────┐
│                       Dual Path Architecture (재정의)                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   [관심사 분리]                                                          │
│   ─────────────                                                          │
...
│   │  ─────────────────────  │    │  ─────────────────────  │            │
│   │                         │    │                         │            │
│   │  • 동기 HTTP            │    │  • Path 1: Temporal     │            │
│   │  • 비동기 HTTP          │    │  • Path 2: Direct       │            │
│   │  • SSE                  │    │                         │            │
│   │  • WebSocket            │    │                         │            │
│   │                         │    │                         │            │
│   │  → 클라이언트가 선택    │    │  → 데이터 특성에 따라   │            │
│   │  → 제한 없음            │    │    시스템이 결정        │            │
│   └─────────────────────────┘    └─────────────────────────┘            │
│                                                                          │
│   ─────────────────────────────────────────────────────────────────      │
...
│   ──────────────────                ─────────────────                   │
│   용도: 오케스트레이션,             용도: Temporal 한계 우회            │
│         상태 관리, 내구성                                                │
│                                     적용 케이스:                         │
│   적용 케이스:                      • 대용량 데이터 (> 2MB)              │
│   • Job 상태 이벤트                 • 고빈도 이벤트 (스트리밍)           │
│   • 단계 완료/실패                  • 저지연 요구사항                    │
│   • 재시도 필요 작업                                                     │
│                                                                          │
│   ─────────────────────────────────────────────────────────────────      │
...
**핵심 결정:**
- **클라이언트 통신 방식(HTTP/SSE/WebSocket)은 제한하지 않는다** — 비즈니스 요구에 따라 선택
- **내부 데이터 경로(Path 1/2)는 데이터 특성에 따라 시스템이 결정**한다
- Temporal 한계를 우회해야 하는 데이터는 Path 2로 라우팅한다
- 두 경로는 **동기화 지점(Sync Point)**에서 상태를 일치시킨다

**상세 결정 위임:**
- Dual Path 구현 상세, 경로 선택 기준, 클라이언트 통신 채널 구현은 **[ADR-005: Dual Path 구현 및 클라이언트 통신 채널 전략](./005_Realtime_Streaming_Channel_Strategy.md)**에서 결정한다
...
| **결과 전달** | 다중 통신 방식 지원 | 클라이언트 선택권 보장, 실시간 UX 지원 |
...
**관련 결정 (본 ADR과 동일 계층에서 참조):**
- [ADR-008: 시스템 클라이언트 인증 및 접근 제어 전략](./008_System_Client_Authentication_and_Access_Control_Strategy.md) — Edge Layer 인증 (3.1절 "인증/인가" 구체화)

**후속 결정 (본 ADR에서 도입한 개념을 구체화):**
- [ADR-004: Universal Connector](./004_Universal_Connector_Strategy_for_Model_Integration.md) — Activity가 Connector를 통해 추론 서버 호출
- [ADR-005: 실시간 스트리밍 채널 전략](./005_Realtime_Streaming_Channel_Strategy.md) — Workflow 내 스트리밍 데이터 전달
- [ADR-006: Worker 리소스 분리](./006_Worker_Resource_Separation_and_Parallel_Processing_Strategy.md) — Worker 유형 분리 (CPU/Accelerator)
- [ADR-007: 추론 서버 배포 경계](./007_Inference_Server_Deployment_and_Worker_Communication_Boundary.md) — Worker와 추론 서버 배포 관계
...
| 2024-12-27 | System Architect | 다중 통신 방식 지원 — SSE 단일 선택에서 클라이언트 선택권(동기/비동기/SSE/WebSocket)으로 변경 |
| 2024-12-27 | System Architect | Dual Path 재정의 — 데이터 경로와 통신 프로토콜을 독립적 관심사로 분리, 상세 구현은 ADR-005로 위임 |
